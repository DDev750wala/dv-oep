...
const BubbleChart = () => {
    ...
    const [dataInFormat, setDataInFormat] =
            useState<ChartData<"bubble", { x: number | Date; y: number; r: number }[], unknown>>();
                ...
                options={{
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                              label: (context) => {
                                const { x, y, r } = context.raw as { x: number | Date; y: number; r: number };
                          
                                const formattedDate =
                                  typeof x === "number" ? new Date(x).toLocaleDateString() : x.toLocaleDateString();
                          
                                return `Date: ${formattedDate}, Sales: $${y.toFixed(2)}, Size: ${r.toFixed(2)}`;
                              },
                            },
                          },                          
                    },
